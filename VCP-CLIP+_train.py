...
import logging 
from models.model_CLIP_prompt import Load_CLIP, tokenize
...
def train(args):
    ...
    beta = torch.nn.Parameter(torch.tensor(0.5, device=device, requires_grad=True))

    optimizer = torch.optim.Adam([{'params': group1 + lr_group1, 'lr': args.learning_rate},
                                  {'params':[alpha], 'lr':4e-3},
                                  {'params':[beta], 'lr':4e-3}], 
        ...
        for epoch in range(epochs):
            ...
            for items in post_dataloader:
                ...
                # with torch.amp.autocast("cuda"):    
                    ...
                    image_features = image_features.detach().clone().requires_grad_()

                    image_features = model.prompt_token_embedding(image_features)

                    ### About Pre-VCP (Context Prompting)
                    # The original VCP-CLIP includes a **Pre-VCP** module (`Context_Prompting`), which modifies the class token using global visual context:
                    """
                    # Original (Pre-VCP enabled)
                    class_token = New_Lan_Embed.before_extract_feat(patch_tokens, image_features, use_global=True)
                    text_embeddings = prompt_pre.forward_ensemble(model, class_token, device)
                    """
                    # Current implementation (Pre-VCP disabled)
                ...
                for num in range(len(anomaly_maps_new)):
                    combined_map = torch.sigmoid(alpha) * anomaly_maps_new[num] + (1 - torch.sigmoid(alpha)) * anomaly_maps_raw[num]
                    combined_map = torch.softmax(combined_map, dim=1)
                    combined_maps.append(combined_map)
                    l_f = loss_focal(combined_map, gt)
                    l_d = loss_dice(combined_map[:, 1, :, :], gt)
                    focal_loss += l_f
                    dice_loss += l_d

                loss_total = torch.sigmoid(beta) * focal_loss + (1 - torch.sigmoid(beta)) * dice_loss 

                optimizer.zero_grad()
                loss_total.backward()
                # IDP debugging
                # print("prompt grad:", model.prompt_token_embedding.prompt_tokens.grad) 
                optimizer.step()

                loss_list.append(loss_total.item())

                print(f"Epoch: {epoch}, Batch: {idx}, Total Loss: {loss_total.item():.4f}")
                
                del patch_tokens_linear, patch_tokens, dense_feature, anomaly_map_raw, anomaly_map_new, combined_map, loss_total, focal_loss, dice_loss
            ...
            if (epoch + 1) % args.save_freq == 0:
                ...
                save_dict = {
                    ...
                    'New_Lan_Embed': New_Lan_Embed.state_dict(), 
                    "Zero_try":Zero_try.state_dict(),
...
if __name__ == '__main__':
    ...
    parser.add_argument("--device_id", type=int, default=0, help="GPU id: >=0")
