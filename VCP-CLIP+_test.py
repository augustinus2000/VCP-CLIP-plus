...
def test(args):
    ...
    alpha_value = torch.sigmoid(alpha).item()
    ...
    for items in test_dataloader:
        ...
        with torch.no_grad(), torch.amp.autocast("cuda", dtype=torch.float16):
            ...
            for num in range(len(anomaly_maps_new)):
                combined_map = alpha_value * anomaly_maps_new[num] + (1 - alpha_value) * anomaly_maps_raw[num]  
                # combined_map이 이미 numpy 배열일 경우, .cpu제거  
                if isinstance(combined_map, torch.Tensor):
                    combined_map = combined_map.cpu().numpy()
            ...
            combined_map = np.mean(combined_maps, axis=0)    

            score_per_img = combined_map.reshape(len(combined_map), -1).max(axis=1)

            results['anomaly_map_raw'].append(anomaly_map_raw)
            results['anomaly_map_new'].append(anomaly_map_new)
            results['combined_map'].append(combined_map)
            results['path'].extend(items['img_path'])
            results['pred_scores'].extend(score_per_img.tolist()) # 추가

    calcuate_metric(results, obj_list, logger, args = args)

    # ========= 카테고리별(Image-level) TP/TN 분석: per-object best threshold =========
    if 'pred_scores' in results:
        all_scores = np.array(results['pred_scores'])  
        all_labels = np.array(results['gt_sp'])        
        ...
        for obj in obj_list:
            ...
            # 카테고리별 best threshold
            precisions, recalls, thresholds = precision_recall_curve(labels, scores)
            if thresholds.size == 0:
                best_thr = 0.5
            else:
                f1_scores = (2 * precisions * recalls) / (precisions + recalls + 1e-8)
                best_thr = thresholds[np.nanargmax(f1_scores)]
        ...
        # ========= 전체 합산 (per-object threshold 적용 후) =========
...
def calcuate_metric(results, obj_list, logger, args = None):
    ...
    combined_map = np.concatenate(results['combined_map'], axis=0)
    ...
    for obj in obj_list: 
        ...
        pr_px_3 = combined_map[object_index,:,:].copy()
        gt_sp = gts_img[object_index].copy()
        gt_px = gts_pixel[object_index,:,:].copy()
        pr_px = normalize(gaussian_filter(pr_px_3, sigma=8, axes = (1,2)))
